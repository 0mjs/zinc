...
func TestCSRFMissingRequestTokenIsBadRequest(t *testing.T) {
	...
	if !errors.Is(got, ErrTokenMissing) {
...
func TestCSRFMissingCookieIsForbidden(t *testing.T) {
	...
	if !errors.Is(got, ErrCookieMissing) {
		t.Fatalf("error=%v", got)
	}
...
func TestCSRFReadersFallBackToMultipartForm(t *testing.T) {
	...
	app.Use(New(Config{
		Generate: fixedCSRFToken("token"),
		Readers: []Reader{
			FromHeader(zinc.HeaderXCSRFToken),
			FromForm("csrf"),
		},
	}))
	app.Post("/submit", func(c *zinc.Context) error {
...
func TestCSRFFromFirstEnforcesPrecedence(t *testing.T) {
	...
	if !errors.Is(got, ErrTokenInvalid) {
		t.Fatalf("error=%v", got)
	}
}
...
func TestCSRFFetchSiteBlocksCrossSite(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "_csrf", Value: "token"})
	req.Header.Set(zinc.HeaderXCSRFToken, "token")
	req.Header.Set(zinc.HeaderSecFetchSite, string(FetchSiteCrossSite))
	rec := httptest.NewRecorder()
	app.ServeHTTP(rec, req)
	...
	if !errors.Is(got, ErrFetchSiteRejected) {
		t.Fatalf("error=%v", got)
	}
}
...
func TestCSRFFetchSiteTrustedOriginAllowsCrossSite(t *testing.T) {
	...
	app.Post("/submit", func(c *zinc.Context) error {
		if MustGet(c).FetchSite != FetchSiteCrossSite {
			t.Fatalf("fetch site=%q", MustGet(c).FetchSite)
		}
		return c.String("ok")
	})
...
func TestCSRFAllowFetchSiteDecider(t *testing.T) {
	t.Run("allows same site decision", func(t *testing.T) {
		...
		app := zinc.New()
		app.Use(New(Config{
			AllowFetchSite: func(_ *zinc.Context, got Decision) (bool, error) {
				decision = got
				return got.Site == FetchSiteSameSite, nil
			},
		}))
		app.Post("/submit", func(c *zinc.Context) error {
			return c.String("ok")
		...
		if decision.Site != FetchSiteSameSite || decision.Origin != "https://app.example.com" || decision.Trusted {
	...
	t.Run("returns decider error", func(t *testing.T) {
		...
		app := zinc.New()
		app.Use(New(Config{
			AllowFetchSite: func(*zinc.Context, Decision) (bool, error) {
...
func TestCSRFViolationErrorMatching(t *testing.T) {
	...
	if got := missing.Error(); !strings.Contains(got, ErrTokenMissing.Error()) {
		t.Fatalf("missing error=%q", got)
	}
	if !errors.Is(missing, ErrTokenMissing) {
	...
	invalid := &Violation{Reason: ReasonTokenInvalid}
	if got := invalid.Error(); got != ErrTokenInvalid.Error() {
		t.Fatalf("invalid error=%q", got)
	}
	if !errors.Is(invalid, ErrTokenInvalid) {
	...
	cookie := &Violation{Reason: ReasonCookieMissing, CookieName: "_csrf"}
	if got := cookie.Error(); !strings.Contains(got, "_csrf") {
		t.Fatalf("cookie error=%q", got)
	}

	fetch := &Violation{Reason: ReasonFetchSiteRejected, FetchSite: FetchSiteCrossSite}
	if got := fetch.Error(); !strings.Contains(got, string(FetchSiteCrossSite)) {
		t.Fatalf("fetch error=%q", got)
	}

	cause := &Violation{Cause: errors.New("custom")}
	if got := cause.Error(); got != "custom" {
		t.Fatalf("cause error=%q", got)
	}

	var nilViolation *Violation
...
func TestCSRFConfigValidation(t *testing.T) {
	assertPanicCSRF(t, func() {
		_ = New(Config{Readers: []Reader{nil}})
	})
	assertPanicCSRF(t, func() {
		_ = New(Config{TrustedOrigins: []string{"https://example.com/path"}})
	})
	assertPanicCSRF(t, func() {
		_ = New(Config{TrustedOrigins: []string{"://bad"}})
	})
}

func fixedCSRFToken(token string) Generator {
