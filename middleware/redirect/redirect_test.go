...
func TestRedirectMiddlewareHelpers(t *testing.T) {
	app := zinc.New()
	app.Use(New(Config{Rules: map[string]string{"/old": "/new"}, StatusCode: http.StatusTemporaryRedirect}))
	...
	app = zinc.New()
	app.Use(New(Config{Rules: map[string]string{"/docs/*": "/new-docs/*"}}))
...
func TestRedirectFallthrough(t *testing.T) {
	app := zinc.New()
	app.Use(New(Config{
