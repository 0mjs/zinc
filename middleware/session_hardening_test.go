...
func sessionTestApp(cfg SessionConfig) *zinc.App {
	app := zinc.New()
	...
	app.Get("/who", func(c *zinc.Context) error { return c.String(MustSession(c).Get("user")) })
	return app
}
...
func TestSessionAuthenticatedExpiryAndRotation(t *testing.T) {
	...
	cfg := SessionConfig{Name: "sid", Secret: oldKey, MaxAge: 100, Now: func() time.Time { return now }}
...
func TestBrowserSessionCookieStillHasServerExpiry(t *testing.T) {
	now := time.Unix(1700000000, 0)
	cfg := SessionConfig{Secret: []byte(strings.Repeat("s", 32)), Lifetime: 2 * time.Second, Now: func() time.Time { return now }}
...
func TestSessionRejectsLegacyUnsignedExpiryAndOversizedInput(t *testing.T) {
	...
	app := sessionTestApp(SessionConfig{Secret: key})
...
func TestSessionMutationBeforeAndAfterCommit(t *testing.T) {
	...
	app.Get("/", func(c *zinc.Context) error {
		...
		if err := s.Set("too-large", strings.Repeat("x", 8192)); !errors.Is(err, ErrSessionTooLarge) {
		...
		if err := s.Set("user", "mallory"); !errors.Is(err, ErrSessionCommitted) {
