...
// Package session keeps string values in a signed client-side cookie.
package session
...
// ErrInvalid identifies a cookie that fails decoding or authentication.
var (
...
const DefaultName = "zinc_session"

// Config controls the signed client-side session cookie. Secret is required;
// everything else defaults to an HTTP-only, SameSite=Lax cookie named
// DefaultName on path "/" that lasts 24 hours.
type Config struct {
	Name string
	// Secret signs the cookie. It must hold at least 32 random bytes.
	Secret []byte
...
// Get returns the request's session.
func Get(c *zinc.Context) (*Session, bool) {
	if c == nil {
		return nil, false
	}
...
// MustGet returns the request's session and panics without the middleware.
func MustGet(c *zinc.Context) *Session {
	session, ok := Get(c)
	if !ok {
		panic("session: session not found")
	}
	return session
}
...
func resolveSessionConfig(config Config) Config {
	cfg := Config{Name: DefaultName, Path: "/", SameSite: http.SameSiteLaxMode}
	if config.Name != "" {
		cfg.Name = config.Name
	}
	if len(config.Secret) < 32 {
		panic("session: Secret must contain at least 32 random bytes")
	}
	cfg.Secret = append([]byte(nil), config.Secret...)
	for _, key := range config.PreviousSecrets {
		if len(key) < 32 {
			panic("session: PreviousSecrets must contain at least 32 random bytes each")
		}
	...
	if cfg.Lifetime < 0 {
		panic("session: Lifetime must not be negative")
	}
	if cfg.Lifetime == 0 {
	...
	if config.MaxAge > 0 {
		if int64(config.MaxAge) > int64((1<<63-1)/time.Second) {
			panic("session: MaxAge is too large")
		}
		cfg.Lifetime = time.Duration(config.MaxAge) * time.Second
	}
	if cfg.Lifetime < time.Second {
		panic("session: Lifetime must be at least one second")
	}
	if config.Path != "" {
...
func decodeSessionCookie(value string, cfg Config) (sessionPayload, bool, error) {
	var decoded sessionPayload
	if len(value) > maxSessionCookieBytes {
		return decoded, false, ErrInvalid
	}
...
func writeSessionCookie(c *zinc.Context, cfg Config, values map[string]string, expires time.Time) error {
	writer := c.Writer()
	...
	cookie := &http.Cookie{Name: cfg.Name, Value: value, Path: cfg.Path, Domain: cfg.Domain, MaxAge: cfg.MaxAge, Secure: cfg.Secure, HttpOnly: !cfg.DisableHTTPOnly, SameSite: cfg.SameSite}
	if cfg.MaxAge > 0 {
