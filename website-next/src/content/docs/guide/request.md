...
Zinc also copies matched parameters into Go's standard `Request.PathValue` storage:
...
Use `FormFiles` for multiple files and `MultipartForm` when you need direct access to every value and file.
